...
describe("parse", () => {
  it("parses a basic connection string", () => {
    const result = parse(
      "llm://api.openai.com/gpt-5.6-sol?temp=0.7&max_tokens=1500",
    );
  ...
  it("parses auth credentials", () => {
    const result = parse(
      "llm://app-name:sk-proj-123456@api.openai.com/gpt-5.6-sol?temp=0.7",
    );
  ...
  it("parses a string with no query params", () => {
    const result = parse("llm://api.openai.com/gpt-5.6-sol");
  ...
  it("expands short provider host aliases", () => {
    const result = parse("llm://openai/gpt-5.6-sol?temp=0.7");
  ...
  it("uses env overrides for short provider host aliases", () => {
    ...
    try {
      const result = parse("llm://openai/gpt-5.6-sol");
  ...
  it("throws on invalid scheme", () => {
    expect(() => parse("http://api.openai.com/gpt-5.6-sol")).toThrow(
      "Invalid scheme",
    );
  });
});
...
describe("build", () => {
  ...
  it("builds with no params", () => {
    ...
    expect(result).toBe("llm://api.openai.com/gpt-5.6-sol");
  });
